...
import type { ConditionPreset } from "@/views/qf/sfc_private/flowable-designer/service/QfdPanelSequenceFlowService";
...
const { form, onConditionPresetChange, commitCondition, commitDefault } =
  QfdPanelSequenceFlowService.useQfdPanelSequenceFlow(
    () => props.modeler,
    () => props.element
  );

function handlePresetChange(val: string): void {
  onConditionPresetChange(val as ConditionPreset);
}
</script>
