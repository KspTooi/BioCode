/**
 * QFD「执行监听器」折叠块：表格行数据、弹窗、增删改、commandStack 刷新
 *
...
export default {
  useQfdPanelExecutionListeners(modelerGetter: () => unknown, elementGetter: () => unknown | null | undefined) {
    const rows = shallowRef<ExecListenerRow[]>([]);
    const dialogVisible = ref(false);
    const editing = ref<ExecListenerRow | null>(null);
    ...
    function isExecutionListener(v: Record<string, unknown>): boolean {
      return v?.$type === "flowable:ExecutionListener";
    }
    ...
    function toRow(listener: Record<string, unknown>): ExecListenerRow {
    ...
    function refresh(): void {
      ...
      rows.value = ext.values.filter((v) => isExecutionListener(v)).map(toRow);
    }
    ...
    function openDialog(row?: ExecListenerRow): void {
      editing.value = row ?? null;
      if (!row) {
        form.value = { event: "start", implKind: "class", implText: "" };
        dialogVisible.value = true;
        return;
      }
      const raw = row.raw;
      form.value.event = (raw.event as string) || "start";
    ...
    function save(): void {
      ...
      if (editRow) {
        modeling.updateModdleProperties(processEl, editRow.raw, propsPatch);
        dialogVisible.value = false;
        refresh();
        return;
      }
      const listener = moddle.create("flowable:ExecutionListener", propsPatch);
    ...
    function removeRow(row: ExecListenerRow): void {
      const m = getM();
      const processEl = elementGetter() as BpmnEl | null;
      if (!m || !processEl?.businessObject) {
        return;
